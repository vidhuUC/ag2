...
## Wiring policies onto an Actor

Pass your policy list via the `assembly=` keyword when constructing an Actor. The Actor wires an internal `AssemblerMiddleware` at the outermost position of the middleware chain automatically.
...
!!! tip
    `AssemblerMiddleware` and `AssemblyPolicy` are also available as public symbols from `#!python autogen.beta` if you need to wire the middleware manually (for example, inside a custom harness that isn't built on `Actor`).
...
Takes no arguments. Dedup state lives on the instance — give each Actor its own `AlertPolicy`.
