...
| Fire every N events | `BatchWatch(N, condition)` |
| Fire once after a delay | `DelayWatch(seconds)` |
| Fire on a schedule | `IntervalWatch(seconds)` or `CronWatch(expr)` |
| Collect events over a time window | `WindowWatch(seconds, condition)` |
...
For event-driven watches (`EventWatch`, `BatchWatch`, `WindowWatch`, `Sequence`), `events` contains the matched events. For time-driven watches (`DelayWatch`, `IntervalWatch`, `CronWatch`), `events` is empty — the trigger is the timer itself.
...
Buffers N matching events, then fires once with the whole batch.

```python linenums="1"
from autogen.beta import BatchWatch
from autogen.beta.events import ModelResponse

# Fire every 5 model responses
watch = BatchWatch(5, condition=ModelResponse)

async def summarize(events, ctx):
    print(f"Batched {len(events)} responses")
```

Any unfilled buffer at disarm-time is discarded.

### WindowWatch

Collects events over a sliding time window, then flushes the batch.

```python linenums="1"
from autogen.beta import WindowWatch
from autogen.beta.events import ModelResponse

watch = WindowWatch(60.0, condition=ModelResponse)

async def hourly_digest(events, ctx):
    print(f"{len(events)} responses in the last minute")
```

The timer starts on the **first** event in a window — a quiet stream produces no firings.
