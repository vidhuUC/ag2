...
Observers let you attach lightweight, read-only event listeners directly to an Actor. Under the hood, each observer is a regular [Stream subscriber](/docs/beta/advanced/stream) — but you register it on the Actor instead of managing the stream yourself.
...
Observer callbacks support the same dependency injection features as [Actor Tools](/docs/beta/tools/tools/) and [stream subscribers](/docs/beta/advanced/stream/) — `Context`, `Inject`, `Depends`, and `Variable`:
...
|---|---|---|---|
| Registration | On Actor | On Actor | On Stream |
...
```python linenums="1"
from autogen.beta import Actor, LoopDetector
from autogen.beta.config import OpenAIConfig
...
```python linenums="1"
from autogen.beta import Actor, TokenMonitor
from autogen.beta.config import OpenAIConfig
